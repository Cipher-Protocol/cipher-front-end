...
class IncrementalQuinTree {
   ...
   */
  public update(_index: number, _value: Leaf) {
...
class MultiIncrementalQuinTree {
  ...
  public insert(_value: Leaf) {
    ...
    this.roots[this.currentTreeNum] = this.hash(
      this.filledSubtrees[this.currentTreeNum][
        this.filledSubtrees[this.currentTreeNum].length - 1
      ]
    );
  }
   ...
   * Update the leaf at the specified index with the given value.
   */
  public update(_absoluteIndex: number, _value: Leaf) {
   ...
   */
  // TODO: write test
  public genMerklePath(_absoluteIndex: number): MerkleProof {
    const capacity = this.leavesPerNode ** this.depth;
  ...
  public copy(): MultiIncrementalQuinTree {
    ...
    newTree.nextIndex = this.nextIndex;
    newTree.filledSubtrees = unstringifyBigInts(
      JSON.parse(JSON.stringify(stringifyBigInts(this.filledSubtrees)))
    );
    newTree.filledPaths = unstringifyBigInts(
      JSON.parse(JSON.stringify(stringifyBigInts(this.filledPaths)))
    );
...
export { IncrementalQuinTree, MultiIncrementalQuinTree };
